...
private:
	...
	[[__gnu__::__cold__]]
#endif
	inline void grow_impl() noexcept
	{
		std::size_t const cap{static_cast<size_type>(end_ptr-begin_ptr)};
		constexpr std::size_t mx_value{std::numeric_limits<std::size_t>::max()/cap};
...
public:
	...
	{
		if(curr_ptr==end_ptr)
#if __has_cpp_attribute(__gnu__::__unlikely__)
		[[__gnu__::__unlikely__]]
#endif
		{
			grow_impl();
		}
